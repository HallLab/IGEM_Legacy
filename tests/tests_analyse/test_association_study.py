...
def python_cat_to_r_cat(python_df):
    ...
    df["Category"] = df["Category"].apply(
        lambda s: (
            s if re_str.match(s) is None else "".join(re_str.match(s).groups())
        )  # noqa E501
    )
...
def compare_loaded(
    ...
    if rtol is not None:
        ...
        assert_frame_equal(
            python_result, loaded_result, atol=0, check_like=True
        )  # noqa E501
...
def test_fpc(data_fpc, design_str, standardize):
    ...
    if design_str == "withoutfpc":
        ...
        # Load the data
        df = igem.load.from_csv(
            DATA_PATH / "fpc_nostrat_data.csv", index_col=None
        )  # noqa E501
        # Process data
    ...
    if not standardize:
        ...
        assert_frame_equal(
            results[regression_kinds[0]], results[regression_kinds[1]]
        )  # noqa E501
    else:
        assert_frame_equal(
            results[regression_kinds[0]],
            results[regression_kinds[1]],
            rtol=1e-04,  # noqa E501
        )
...
def test_api(design_str, standardize):
    ...
    if design_str == "noweights":
        ...
        df = igem.modify.make_continuous(df, only=["api00", "ell", "meals", "mobility"])
        ...
        df = igem.load.from_csv(
            DATA_PATH / "apipop_withna_data.csv", index_col=None
        )  # noqa E501
        df = igem.modify.make_continuous(df, only=["api00", "ell", "meals", "mobility"])
        df = igem.modify.colfilter(
            df, only=["api00", "ell", "meals", "mobility"]
        ...
        df = igem.load.from_csv(
            DATA_PATH / "apistrat_data.csv", index_col=None
        )  # noqa E501
        df = igem.modify.make_continuous(df, only=["api00", "ell", "meals", "mobility"])
        design = igem.survey.SurveyDesignSpec(
        ...
        df = igem.load.from_csv(
            DATA_PATH / "apiclus1_data.csv", index_col=None
        )  # noqa E501
        df = igem.modify.make_continuous(df, only=["api00", "ell", "meals", "mobility"])
        design = igem.survey.SurveyDesignSpec(
...
def test_nhanes_fulldesign(data_NHANES, standardize):
    ...
    if not standardize:
        compare_loaded(
            results[regression_kinds[0]], surveylib_result_file, rtol=1e-04
        )  # noqa E501
        assert_frame_equal(
            results[regression_kinds[0]],
            results[regression_kinds[1]],
...
def test_nhanes_fulldesign_subset_category(data_NHANES, standardize):
    ...
    if not standardize:
        compare_loaded(
            results[regression_kinds[0]], surveylib_result_file, rtol=1e-03
        )  # noqa E501
        assert_frame_equal(
            results[regression_kinds[0]],
            results[regression_kinds[1]],
...
def test_nhanes_fulldesign_subset_continuous(standardize):
    ...
    # Load the data
    df = igem.load.from_csv(
        DATA_PATH / "nhanes_data_subset.csv", index_col=None
    )  # noqa E501
    # Process data
    ...
    df = igem.modify.colfilter(
        df, only=["HI_CHOL", "RIAGENDR", "race", "agecat"]
    )  # noqa E501
    # Get Results
    surveylib_result_file = (
...
def test_nhanes_lonely(
    data_NHANES_lonely, single_cluster, load_filename, standardize
):  # noqa E501
...
def test_nhanes_realistic(standardize):
    ...
    df = igem.load.from_tsv(
        DATA_PATH.parent / "data_files" / "nhanes_real.txt", index_col="ID"  # noqa E501
    )
    # Process data
...
def test_nhanes_subset_singleclusters(standardize):
    ...
    # Load the data
    df = igem.load.from_tsv(
        DATA_PATH.parent / "data_files" / "nhanes_subset" / "data.txt"
    )
    survey_df = igem.load.from_tsv(
        DATA_PATH.parent
        / "data_files"
        / "nhanes_subset"
        / "design_data.txt"  # noqa E501
    )
