...
def load_r_interaction_results(filename):
    """Load directly calculated results (from R) and convert column names to match python results"""  # noqa E501
    ...
    if "Parameter" in r_result.columns:
        r_result = r_result.set_index(
            ["Term1", "Term2", "Parameter", "Outcome"]
        )  # noqa E501
    else:
...
def compare_result(loaded_result, python_result, atol=0, rtol=1e-04):
    """Compare loaded results to igem results, with optional tolerances"""
    ...
    if "Parameter" in python_result.index.names:
        ...
        python_result["Parameter"] = python_result["Parameter"].apply(
            fix_python_param
        )  # noqa E501
    ...
    for var in ["Full_Var1_Var2_SE", "Full_Var1_Var2_Pval"]:  # noqa E501
        try:
            assert np.allclose(
...
def test_interactions_nhanes_ageXgender(data_NHANES):
    ...
    loaded_result = load_r_interaction_results(
        RESULT_PATH / "nhanes_ageXgender.csv"
    )  # noqa E501
...
def test_interactions_nhanes_weightXrace(data_NHANES):
    """
    Test the nhanes dataset a specific interaction (using 'weight' as a continuous variable).  # noqa E501
    ...
    # No Betas
    loaded_result = load_r_interaction_results(
        RESULT_PATH / "nhanes_weightXrace.csv"
    )  # noqa E501
    python_result = igem.analyze.interaction_study(
        outcomes="HI_CHOL",
...
def test_interactions_nhanes_pairwise(data_NHANES):  # noqa E501
    """Test the nhanes dataset with pairwise interactions of the three variables"""  # noqa E501
    # Process data
    df = igem.modify.colfilter(
        data_NHANES, only=["HI_CHOL", "RIAGENDR", "race", "agecat"]
    )
    ...
    # No Betas
    loaded_result = load_r_interaction_results(
        RESULT_PATH / "nhanes_pairwise.csv"
    )  # noqa E501
    ...
    # Test Adding pvalues
    igem.analyze.add_corrected_pvalues(
        python_result_nobeta, pvalue="LRT_pvalue"
    )  # noqa E501
    # igem.analyze.add_corrected_pvalues(python_result, pvalue="Beta_pvalue")
    igem.analyze.add_corrected_pvalues(
        python_result, pvalue="Full_Var1_Var2_beta"
    )  # noqa E501
    igem.analyze.add_corrected_pvalues(
    ...
    #     .first()
    # )
    # assert (grouped_bonf == python_result_nobeta["LRT_pvalue_bonferroni"]).all()  # noqa E501
