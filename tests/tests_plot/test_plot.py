...
DATA_PATH = Path(__file__).parent.parent / "data_files"
...
def test_manhattan(
    result_list, bonferroni, fdr, label_vars, categories, request
):  # noqa E501
...
def test_distributions(dataNHANESReal, filename, quality, tmpdir):
    ...
    igem.plot.distributions(
        data=dataNHANESReal, filename=filename, quality=quality
    )  # noqa E501
