...
def test_colfilter_percent_zero(plantTraits, capfd):
    ...
    assert len(result) == len(
        result
    )  # TODO: Check another returns informations  # noqa E501
    # out, err = capfd.readouterr()
    # assert (
    #     out
    #     == "================================================================================\n"  # noqa E501
...
def test_colfilter_min_cat_n(plantTraits, capfd):
    ...
    plantTraits = modify.make_categorical(
        data=plantTraits, only=["test", "test2"]
    )  # noqa E501
...
def test_remove_outliers_gaussian(plantTraits):
    ...
    result = modify.remove_outliers(
        plantTraits, method="gaussian", skip=["durflow"]
    )  # noqa E501
    assert (
        result.isna().sum()["longindex"] == plantTraits.isna().sum()["longindex"]
    )  # noqa E501
    assert (
        result.isna().sum()["durflow"] == plantTraits.isna().sum()["durflow"]
    )  # noqa E501
    assert (
        result.isna().sum()["vegaer"] == plantTraits.isna().sum()["vegaer"] + 12
    )  # noqa E501
    return
...
def test_remove_outliers_iqr(plantTraits):
    ...
    assert (
        result.isna().sum()["vegaer"] == plantTraits.isna().sum()["vegaer"] + 17
    )  # noqa E501
    return
